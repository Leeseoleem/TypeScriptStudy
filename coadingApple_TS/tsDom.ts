// 1. ts로  HTML 조작하기
let 제목 = document.querySelector("#title"); // CSS 관련 요소 조작
let 소제목 = document.querySelector("#subTitle");
// 제목?.innerHTML = "반갑습니다"; => 에러: 보통 HTML의 요소 => null type을 기본으로 가짐
// 방법 1. as로 타입 확정시키기
let 메인 = document.querySelector("#main") as Element; // 물론 as는 권장하지 않는다
...
let 젤다 = document.querySelectorAll(".naver");
젤다.forEach((a) => {
...
let 버튼조작 = document.querySelector("#button");
let 이미지변환 = document.querySelector("#image");
버튼조작?.addEventListener("click", function () {
